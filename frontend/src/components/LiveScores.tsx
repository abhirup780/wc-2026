...
function GoalLine({ g }: { g: ESPNGoal }) {
  return (
    <span className="flex items-center gap-1 text-[11px] text-gray-400 leading-snug">
      <span className="text-gray-600">⚽</span>
      <span className="text-gray-200">{g.scorer.split(' ').slice(-1)[0]}</span>
      {g.type === 'own-goal' && <span className="text-red-400 text-[10px]">og</span>}
      {g.type === 'penalty'  && <span className="text-amber-400 text-[10px]">p</span>}
      <span className="text-gray-600 tabular-nums">{g.minute}</span>
    </span>
  );
}
        ...
        <div className="flex gap-2 pt-3 mt-3 border-t border-white/5">
          ...
          <div className="flex-1 flex flex-col items-end gap-0.5">
            {away.length ? away.map((g, i) => (
              <span key={i} className="flex items-center gap-1 text-[11px] text-gray-400 leading-snug">
                <span className="text-gray-600 tabular-nums">{g.minute}</span>
                {g.type === 'penalty'  && <span className="text-amber-400 text-[10px]">p</span>}
                {g.type === 'own-goal' && <span className="text-red-400 text-[10px]">og</span>}
                <span className="text-gray-200">{g.scorer.split(' ').slice(-1)[0]}</span>
...
function LiveCard({ m }: { m: RichMatch }) {
  ...
  return (
    <div className="relative rounded-2xl overflow-hidden"
      style={{ background: 'linear-gradient(160deg, #052e16 0%, #111827 60%)',
        boxShadow: '0 0 40px rgba(22,163,74,0.18), inset 0 1px 0 rgba(74,222,128,0.15)' }}>
      {/* top accent line */}
      ...
      <div className="p-5">
        ...
        <div className="flex items-center justify-center gap-3 mb-4">
          <span className="text-xs text-gray-500 uppercase tracking-widest">{stageLabel(m)}</span>
          <span className="flex items-center gap-1.5 bg-green-950/80 border border-green-800/60 rounded-full px-2.5 py-0.5">
            <span className="live-dot" />
            <span className="text-green-400 text-xs font-semibold tabular-nums">{m.clock || 'LIVE'}</span>
          </span>
        </div>
        ...
        <div className="flex items-center">
          ...
          <div className="flex-1 flex flex-col items-center gap-2">
            ...
            <div className="text-center">
              <p className={`text-sm font-bold leading-tight ${hW ? 'text-white' : 'text-gray-400'}`}>{teamName(m.homeId)}</p>
              <p className="text-[10px] font-mono text-gray-600 tracking-widest mt-0.5">{m.homeId}</p>
            </div>
          </div>
          ...
          <div className="flex flex-col items-center px-4">
            <div className="font-mono font-black tabular-nums leading-none"
              ...
              <span className={hW ? 'text-white' : 'text-gray-500'}>{m.homeGoals ?? '–'}</span>
              <span className="text-gray-700 mx-3">:</span>
              <span className={aW ? 'text-white' : 'text-gray-500'}>{m.awayGoals ?? '–'}</span>
            </div>
          </div>
          ...
          <div className="flex-1 flex flex-col items-center gap-2">
            ...
            <div className="text-center">
              <p className={`text-sm font-bold leading-tight ${aW ? 'text-white' : 'text-gray-400'}`}>{teamName(m.awayId)}</p>
              <p className="text-[10px] font-mono text-gray-600 tracking-widest mt-0.5">{m.awayId}</p>
            </div>
          </div>
        </div>
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 px-3 py-3">
        <span className="hidden sm:block text-[10px] text-gray-600 w-12 flex-shrink-0 uppercase tracking-wider text-center">
          {stageLabel(m)}
        </span>
        ...
        <div className="flex items-center gap-2 flex-1 justify-end min-w-0">
          <div className="text-right min-w-0">
            <p className={`text-sm font-semibold truncate leading-tight ${hW ? 'text-white' : 'text-gray-400'}`}>
              {teamName(m.homeId)}
            </p>
            <p className="text-[10px] font-mono text-gray-600 tracking-widest">{m.homeId}</p>
          </div>
        ...
        <div className="flex flex-col items-center w-20 flex-shrink-0 select-none">
          <div className="font-mono text-xl font-black tabular-nums leading-none">
            <span className={hW ? 'text-white' : 'text-gray-500'}>{m.homeGoals ?? '–'}</span>
            <span className="text-gray-700 mx-1.5">–</span>
            <span className={aW ? 'text-white' : 'text-gray-500'}>{m.awayGoals ?? '–'}</span>
          </div>
          <span className="text-[10px] text-gray-600 mt-0.5 font-medium tracking-wide">FT</span>
        </div>
        ...
        <div className="flex items-center gap-2 flex-1 min-w-0">
          ...
          <div className="min-w-0">
            <p className={`text-sm font-semibold truncate leading-tight ${aW ? 'text-white' : 'text-gray-400'}`}>
              {teamName(m.awayId)}
            </p>
            <p className="text-[10px] font-mono text-gray-600 tracking-widest">{m.awayId}</p>
          </div>
        </div>

        {/* Chevron / spinner */}
        {showToggle && (
          <span className={`text-gray-600 text-xs flex-shrink-0 ml-1 transition-transform duration-200 ${open ? 'rotate-180' : ''}`}>
            {loading ? '·' : '▾'}
          </span>
        )}
      </div>
      ...
      {open && (
        <div className="px-3 pb-3">
          {loading ? (
            <div className="pt-3 mt-3 border-t border-white/5 text-[11px] text-gray-600 text-center">Loading scorers…</div>
          ) : hasGoals ? (
            <ScorerDrawer goals={goals!} open />
          ) : goals !== null ? (
            <div className="pt-3 mt-3 border-t border-white/5 text-[11px] text-gray-600 text-center">No scorer data available</div>
          ) : null}
        </div>
      )}
    </div>
...
function UpcomingCard({ m, accent }: { m: RichMatch; accent?: boolean }) {
  ...
  const isSoon  = new Date(m.kickoffUtc).getTime() - Date.now() < 3 * 36e5;
  const hi = accent && (isToday || isSoon);
  return (
    <div className={`rounded-xl border px-3 py-3 flex items-center gap-2 transition-colors duration-150
      ...
      <div className="flex flex-col items-center w-20 flex-shrink-0">
        <span className={`text-xs font-semibold tabular-nums leading-tight text-center
          ${hi ? 'text-blue-300' : 'text-gray-500'}`}>
          {isToday ? countdown(m.kickoffUtc) : formatKickoff(m.kickoffUtc)}
        </span>
        <span className="text-[10px] text-gray-700 mt-0.5">vs</span>
      </div>
...
function SectionHeader({ label, count, collapsible, collapsed, onToggle }:
  ...
  return (
    <button className="flex items-center gap-3 w-full text-left group py-1"
      ...
      <span className="text-[11px] font-bold uppercase tracking-widest text-gray-500 group-hover:text-gray-300 transition-colors">
        {label}
      </span>
      <span className="text-[11px] text-gray-700 font-mono">{count}</span>
    ...
    <div className="space-y-7 max-w-2xl mx-auto">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          ...
          {hasLive && (
            <span className="flex items-center gap-1.5 bg-green-950 border border-green-800/60 text-green-400 text-[11px] font-semibold px-2.5 py-1 rounded-full">
            ...
            <div className="space-y-5">
              {[...futureByDay.entries()].map(([, ms]) => (
                <div key={localDay(ms[0].kickoffUtc)} className="space-y-1.5">
                  <p className="text-[11px] text-gray-600 font-semibold pl-1 uppercase tracking-widest">
